...
import { CHANNEL, PALETTEN, VIDEO, VOICE } from "./config.js";
...
export async function rendere({ skript, voicePfad, wordsPfad, text }) {
  ...
  if (clip) {
    ...
    const gradient =
      `gradients=s=${VIDEO.breite}x${VIDEO.hoehe}:c0=${p.c0}:c1=${p.c1}:c2=${p.c2}` +
      `:n=3:type=${p.type}:speed=0.004:d=30:r=${VIDEO.fps}:seed=${Math.floor(Math.random() * 1e6)}`;
  ...
  const bgFilter = clip
    ...
    : `[1:v]format=yuv420p,eq=brightness=-0.04:saturation=1.25,vignette=PI/4,setsar=1[bgv]`;

  const filter = [
    bgFilter,
    `[bgv]subtitles=${assPfad}:fontsdir=assets/fonts[v]`,
